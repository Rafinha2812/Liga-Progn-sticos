...
# ---------------------------------------------------------
# SISTEMA DE MEMÓRIA DE SESSÃO COM SINCRONIZAÇÃO SUPABASE
# ---------------------------------------------------------
if "resultados_dados" not in st.session_state:
    st.session_state.resultados_dados = {}
    try:
        res = supabase.table("jornadas").select("*").execute()
        if res and hasattr(res, "data") and res.data:
            for item in res.data:
                if isinstance(item, dict) and item.get("id_jogo"):
                    st.session_state.resultados_dados[item["id_jogo"]] = {
...
if "palpites_dados" not in st.session_state:
    st.session_state.palpites_dados = {}
    try:
        ...
        if res and hasattr(res, "data") and res.data:
            for item in res.data:
                if isinstance(item, dict):
                    ...
                    if jog and id_j:
                        if jog not in st.session_state.palpites_dados:
                            st.session_state.palpites_dados[jog] = {}
                        st.session_state.palpites_dados[jog][id_j] = {
                            "c": item.get("p_casa", 0),
                            "f": item.get("p_fora", 0)
                        }
    except Exception:
...
def obter_mes_por_jornada(j):
    j = int(j)
    if j in [1, 2]: return "Setembro 2026"
    elif j in [3, 4, 5, 6]: return "Outubro 2026"
    elif j in [7, 8, 9, 10]: return "Novembro 2026"
    elif j in [11, 12, 13]: return "Dezembro 2026"
    elif j in [14, 15, 16]: return "Janeiro 2027"
    elif j in [17, 18, 19, 20]: return "Fevereiro 2027"
    elif j in [21, 22, 23, 24]: return "Março 2027"
    elif j in [25, 26, 27, 28]: return "Abril 2027"
    elif j in [29, 30]: return "Maio 2027"
    return "Outro"
...
aba = st.sidebar.radio("Navegação", ["📊 Classificações", "📝 Inserir Palpites", "⚙️ Painel Admin"])
...
if aba == "📊 Classificações":
    ...
    if not jogadores:
        ...
        sub_aba1, sub_aba2, sub_aba3 = st.tabs(["🏆 Geral (Época)", "📅 Por Mês", "⚽ Por Jornada"])

        with sub_aba1:
            st.subheader("Classificação Geral Acumulada")
            pts_t, ex_t, tend_t = {j: 0 for j in jogadores}, {j: 0 for j in jogadores}, {j: 0 for j in jogadores}
            ...
            }).sort_values(by="Pontos Totais", ascending=False).reset_index(drop=True)
            df_g.index += 1
            st.table(df_g)
        ...
        with sub_aba2:
            ...
            pts_m, ex_m, tend_m = {j: 0 for j in jogadores}, {j: 0 for j in jogadores}, {j: 0 for j in jogadores}
            for j_num in CALENDARIO_LOCAL.keys():
            ...
            }).sort_values(by=f"Pontos ({mes_sel})", ascending=False).reset_index(drop=True)
            df_m.index += 1
            st.table(df_m)
        ...
        with sub_aba3:
            ...
            pts_j, ex_j, tend_j = {j: 0 for j in jogadores}, {j: 0 for j in jogadores}, {j: 0 for j in jogadores}
            ...
            }).sort_values(by=f"Pontos J{j_sel_tab}", ascending=False).reset_index(drop=True)
            df_j.index += 1
            st.table(df_j)
...
# ---------------------------------------------------------
    ...
    opcao_jog = st.radio("Identificação do Apostador:", ["Apostador Existente", "Novo Apostador"], horizontal=True)
    ...
    if opcao_jog == "Apostador Existente":
        if lista_existentes:
            ...
            st.warning("Ainda não existem apostadores registados. Seleciona 'Novo Apostador' para criar o primeiro.")
    else:
    ...
    if nome:
        ...
        with st.form("form_palpites"):
            ...
            for jogo in jogos_j:
                ...
                with col1: st.write(f"**{jogo['casa']}**")
                with col2: pc = st.number_input("", min_value=0, max_value=15, value=p_ant["c"], key=f"p_c_{id_j}")
                with col3: pf = st.number_input("", min_value=0, max_value=15, value=p_ant["f"], key=f"p_f_{id_j}")
                with col4: st.write(f"**{jogo['fora']}**")
                novos_p[id_j] = {"c": pc, "f": pf}

            if st.form_submit_button("Guardar Prognósticos"):
                # 1. Atualizar imediatamente na memória da aplicação
                if nome not in st.session_state.palpites_dados:
                    st.session_state.palpites_dados[nome] = {}
                for id_j, val in novos_p.items():
                    st.session_state.palpites_dados[nome][id_j] = {"c": val["c"], "f": val["f"]}

                # 2. Enviar para o Supabase em segundo plano
                try:
                    registos = []
                    for id_j, val in novos_p.items():
                        registos.append({"jogador": nome, "id_jogo": id_j, "p_casa": val["c"], "p_fora": val["f"]})
                    ids_j = list(novos_p.keys())
                    supabase.table("palpites").delete().eq("jogador", nome).in_("id_jogo", ids_j).execute()
    ...
    with sub1:
        ...
        with st.form("form_admin"):
            ...
            for jogo in CALENDARIO_LOCAL.get(str(j_sel), []):
                ...
                val_c = 0 if res_ant.get("res_casa") is None else int(res_ant["res_casa"])
                val_f = 0 if res_ant.get("res_fora") is None else int(res_ant["res_fora"])

                col1, col2, col3, col4 = st.columns([3, 1, 1, 3])
                with col1: st.write(f"**{jogo['casa']}**")
                with col2: rc = st.number_input("", min_value=0, max_value=15, value=val_c, key=f"r_c_{id_j}")
                with col3: rf = st.number_input("", min_value=0, max_value=15, value=val_f, key=f"r_f_{id_j}")
                with col4: st.write(f"**{jogo['fora']}**")
                novos_res.append({"id_jogo": id_j, "c": rc, "f": rf})

            marcar_fechado = st.checkbox("Marcar TODOS os jogos desta jornada como concluídos")
            ...
            if st.form_submit_button("Guardar Resultados"):
                ...
                for nr in novos_res:
                    ...
                    st.session_state.resultados_dados[nr["id_jogo"]] = {"res_casa": res_c, "res_fora": res_f}
